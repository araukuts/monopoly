...
- (void)gameManager:(MNPGameManager *)gameManager
  ...
  [self createAlertViewForRollingWithRollDice:rollDice];
...
// Combine animations
  ...
  [groupAnimation setValue:@"moving" forKey:@"animationID"];
...
- (void)createAlertViewForRollingWithRollDice:(NSInteger)rollDice {

  UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Roll!"
                                                      message:[NSString stringWithFormat:@"Rolled %ld",(long)rollDice]
                                                     delegate:self
                                            cancelButtonTitle:@"Ok"
                                            otherButtonTitles: nil];
  [alertView show];
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag {
  ...
  if (flag) {
    ...
    if (_someCount == 3) {
      ...
      --_someCount;
      if (_someCount != 0) return;
      NSNumber *rollDice = @(2 + 2);
